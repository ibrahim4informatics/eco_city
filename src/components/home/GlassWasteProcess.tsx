    ...
    return (
        <Box w={"100%"} px={2} minH={"100dvh"} py={4} display={"flex"} justifyContent={"center"} alignItems={"center"}>
            ...
            <Box w={"100%"} maxW={1024} display={"flex"} gap={2} flexDir={{ base: "column", lg: "row" }}>
                <Box flex={1}>
                    <Heading viewport={{ once: true }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: .4 }} initial={{ opacity: 0, y: -100 }} size={"4xl"}>🪟 What Happens to Glass Waste?</Heading>
                    <Heading viewport={{ once: true }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: .4 }} initial={{ opacity: 0, x: -100 }} pos={"relative"} _before={{ content: "''", pos: "absolute", h: "100%", w: 1, bg: "accent.400", top: 0, left: 0 }} px={2} size={"xl"} my={2} color={"GrayText"}>Melting glass for a sparkling new start!</Heading>
                ...
                <Box flex={1}>
                    <Image viewport={{ once: true }} whileInView={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.3, delay: .4 }} initial={{ opacity: 0, y: -100, scale: 0 }} src={pic} w={"100%"} rounded={"md"} />
                </Box>
            </Box>
